...
int main(){
	...
	ll deli_value = 0,curr_weight = 0;
	for(int i=0;i < n;i++){
		cin >> cheese[i].first >> cheese[i].second;
		deli_value += cheese[i].first* cheese[i].second;
		curr_weight += cheese[i].second;
	}
	
	if(curr_weight <= w) cout << deli_value << endl;
	else{
		sort(cheese.begin(),cheese.end());

		for(int i=0;i < n && curr_weight > w;i++){
			if(curr_weight - cheese[i].second >= w){
				curr_weight -= cheese[i].second;
				deli_value -= cheese[i].first*cheese[i].second;
			}else{
				deli_value -= cheese[i].first*(curr_weight-w);
				curr_weight -= (curr_weight-w);
			}
		}
		cout << deli_value << endl;
	}
